...
class AdminSystem {
    ...
    async loadUsers() {
        try {
            const response = await fetch(`${this.apiAdminBase}/users`, {
                method: 'POST',
                headers: {
    ...
    async approveDecorator(userId, approved) {
        try {
            const response = await fetch(`${this.apiAdminBase}/decorator/approve`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async editUser(userId) {
        try {
            ...
            const response = await fetch(`${this.apiAdminBase}/user`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async saveUserEdit() {
        try {
            ...
            const response = await fetch(`${this.apiAdminBase}/user/update`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    setupEventListeners() {
        ...
        if (adminBio) {
            adminBio.addEventListener('input', () => this.updateAdminBioCounter());
        }

        const profileInput = document.getElementById('admin-profile-photo-input');
        if (profileInput) {
            profileInput.addEventListener('change', (event) => this.handleAdminPhotoChange(event));
        }

        const removePhotoBtn = document.getElementById('remove-admin-photo');
        if (removePhotoBtn) {
            removePhotoBtn.addEventListener('click', () => this.removeAdminPhoto());
        }

        document.querySelectorAll('.password-toggle').forEach(button => {
            button.addEventListener('click', () => this.togglePasswordInput(button));
        });

        const resetProfileBtn = document.getElementById('reset-admin-profile');
        if (resetProfileBtn) {
            resetProfileBtn.addEventListener('click', (e) => {
                e.preventDefault();
                this.resetAdminProfileForm();
            });
        }
    ...
    async loadDashboardData() {
        ...
        try {
            const response = await fetch(`${this.apiAdminBase}/dashboard`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async createDecorator() {
        ...
        // Validações
        if (!this.validateDecoratorData(legacyValidatorData)) {
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`${this.apiAdminBase}/decorator/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok && result.success && result.data) {
                form.reset();
                this.showNotification(result.message || 'Decorador criado com sucesso!', 'success');
    ...
    async loadPageCustomization(decoratorId) {
        try {
            const response = await fetch(`${this.apiAdminBase}/page/customization`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async savePageCustomization() {
        try {
            ...
            const response = await fetch(`${this.apiAdminBase}/page/customization/save`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    previewPageCustomization() {
        ...
        if (user && user.slug) {
            window.open(`${window.location.origin}/decorador/${user.slug}`, '_blank');
        } else {
            this.showNotification('Slug do decorador não encontrado', 'error');
        }
    }

    async loadAdminProfile(force = false) {
        if (this.adminSettingsLoaded && !force) {
            return;
        }

        try {
            const response = await fetch(`${this.apiAdminBase}/profile`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok && result.success && result.data) {
                ...
                this.updateAdminProfileUI(result.data);
                this.adminSettingsLoaded = true;
                this.pendingAdminProfileImage = null;
                this.removeAdminProfileImage = false;
                const fileInput = document.getElementById('admin-profile-photo-input');
                if (fileInput) fileInput.value = '';
                this.updateAdminBioCounter();
            } else {
                this.showNotification(result.message || 'Não foi possível carregar o perfil do administrador.', 'error');
            }
        } catch (error) {
            console.error('Erro ao carregar perfil do admin:', error);
            this.showNotification('Erro de conexão ao carregar perfil administrativo.', 'error');
        }
    }

    updateAdminProfileUI(profile = {}) {
        document.getElementById('admin-profile-name').textContent = profile.name || 'Administrador';
        document.getElementById('admin-profile-email').textContent = profile.email || '';
        ...
        const instagramInput = document.getElementById('admin-instagram');
        if (instagramInput) instagramInput.value = profile.instagram || '';

        const communicationEmailInput = document.getElementById('admin-communication-email');
        if (communicationEmailInput) communicationEmailInput.value = profile.communication_email || '';

        const bioInput = document.getElementById('admin-bio');
        if (bioInput) bioInput.value = profile.bio || '';

        this.updateAdminBioCounter();
        this.updateAdminPhotoPreview(profile.profile_photo_url);
    }
    ...
    resetAdminProfileForm() {
        ...
        this.updateAdminProfileUI(this.adminProfile);
        this.pendingAdminProfileImage = null;
        this.removeAdminProfileImage = false;
        ...
        this.showAdminPhotoFeedback('Alterações descartadas.', 'info');
    }

    updateAdminBioCounter() {
        const bioInput = document.getElementById('admin-bio');
    ...
    handleAdminPhotoChange(event) {
        ...
        reader.onload = (e) => {
            ...
            this.removeAdminProfileImage = false;
            this.updateAdminPhotoPreview(this.pendingAdminProfileImage);
            this.showAdminPhotoFeedback('Pré-visualização atualizada.', 'success');
        };
    ...
    removeAdminPhoto() {
        ...
        const fileInput = document.getElementById('admin-profile-photo-input');
        if (fileInput) fileInput.value = '';
        this.updateAdminPhotoPreview(null);
        this.showAdminPhotoFeedback('A foto será removida ao salvar.', 'warning');
    }

    updateAdminPhotoPreview(src) {
        const image = document.getElementById('admin-profile-photo');
        if (!image) return;
        image.src = src || this.defaultAdminPhoto;
    }

    showAdminPhotoFeedback(message, type = 'info') {
        ...
        feedback.className = `mt-4 text-xs ${colors[type] || colors.info}`;
    ...
    showAdminProfileUpdated() {
        const badge = document.getElementById('admin-profile-updated');
        if (!badge) return;
    ...
    async saveAdminProfile() {
        if (!this.adminProfile) {
            await this.loadAdminProfile(true);
        }
        ...
        if (this.pendingAdminProfileImage) {
            payload.profile_photo = this.pendingAdminProfileImage;
        }

        if (this.removeAdminProfileImage) {
        ...
        try {
            const response = await fetch(`${this.apiAdminBase}/profile/update`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok && result.success && result.data) {
                ...
                const fileInput = document.getElementById('admin-profile-photo-input');
                if (fileInput) fileInput.value = '';
                this.showAdminProfileUpdated();
                this.showNotification(result.message || 'Perfil atualizado com sucesso!', 'success');
            } else {
                this.showNotification(result.message || 'Erro ao atualizar perfil.', 'error');
            }
        } catch (error) {
            console.error('Erro ao salvar perfil do admin:', error);
            this.showNotification('Erro de conexão ao salvar perfil.', 'error');
        } finally {
            submitBtn.innerHTML = originalText;
            submitBtn.disabled = false;
        }
    ...
    async changeAdminPassword() {
        ...
        if (newPassword !== confirmPassword) {
            this.showNotification('As senhas informadas não conferem.', 'error');
            return;
        }
        ...
        try {
            const response = await fetch(`${this.apiAdminBase}/profile/change-password`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok && result.success) {
                this.showNotification(result.message || 'Senha atualizada com sucesso!', 'success');
                form.reset();
            } else {
                this.showNotification(result.message || 'Erro ao atualizar senha.', 'error');
            }
        } catch (error) {
            console.error('Erro ao alterar senha do admin:', error);
            this.showNotification('Erro de conexão ao atualizar senha.', 'error');
        } finally {
            submitBtn.innerHTML = originalText;
            submitBtn.disabled = false;
        }
