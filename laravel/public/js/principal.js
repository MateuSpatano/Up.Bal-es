...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function saveAccountData() {
        ...
        try {
            const response = await fetch(`${DECORATOR_API_BASE}/account`, {
                method: 'POST',
                body: formData,
                credentials: 'include'
            });
...
async function loadContactInfo() {
    try {
        ...
        if (contactSlug) {
            params.set('slug', contactSlug);
        }

        const response = await fetch(`${PUBLIC_API_BASE}/contacts${params.toString() ? `?${params.toString()}` : ''}`, {
            credentials: 'omit'
        });
        const result = await response.json();
