...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const PUBLIC_BUDGET_ENDPOINT = `${PUBLIC_API_BASE}/budgets`;
    const decoratorId = form?.dataset?.decoratorId || null;
    ...
    async function submitServiceRequest(formData) {
        try {
            ...
            if (decoratorId && !formData.has('decorator_id')) {
                formData.append('decorator_id', decoratorId);
            }

            const response = await fetch(PUBLIC_BUDGET_ENDPOINT, {
                method: 'POST',
                body: formData,
                credentials: 'omit'
            });
